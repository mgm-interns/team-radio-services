...
@Log4j2
@MessageEndpoint
public class SendResetPasswordMailEvent extends BaseEventMessageEndpoint {
    ...
    @Value("classpath:mailTemplate/forgot-password-email.html")
    private Resource forgotPasswordTemplateMail;

    public SendResetPasswordMailEvent(MailgunHelper mailgunHelper,
    ...
    protected void process(Map<String, Object> message) {
        ...
        try {
            if (user.isPresent()
                    && !StringUtils.isEmpty(user.get().getEmail())
                    && !StringUtils.isEmpty(user.get().getResetPasswordToken())) {

                StringBuilder resetPasswordUrlBuilder = new StringBuilder();
                resetPasswordUrlBuilder.append(FRONT_END_URL)
                        .append(RESET_PASSWORD_URL)
                        .append(user.get().getResetPasswordToken());

                String mailContent = IOUtils.toString(forgotPasswordTemplateMail.getInputStream())
                        .replace("{{reset_password_url}}", resetPasswordUrlBuilder.toString())
                        .replace("{{name}}", user.get().getName());

                String subject = messageSource.getMessage("mail.forgot_password.subject",
                        new String[]{},
                        Locale.getDefault());
    ...
    private boolean isValid(String userId, String eventId) {
        return (!StringUtils.isEmpty(userId)
                ...
                && eventId.equals(SubscriptionEvents.forgot_password.name()));
    }
}
